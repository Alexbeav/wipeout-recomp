...
#
# Writes: dist/wipeout-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix wipeout-recomp \
  --exe-name WipEout_Recompiled \
  --display-name "WipEout Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned WipEout disc" \
  --bios-hint "a legal SCPH-5552 BIOS dump (required; OpenBIOS is not supported)" \
  --omit-openbios \
  --runtime-dir mods \
